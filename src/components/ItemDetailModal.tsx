  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-zinc-950/60 backdrop-blur-xs animate-fade-in">
      ...
      <div className="relative z-10 bg-white w-full sm:max-w-md rounded-t-[32px] sm:rounded-[32px] shadow-2xl border border-zinc-100 overflow-hidden flex flex-col max-h-[92vh] sm:max-h-[88vh] animate-in slide-in-from-bottom sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-200">
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-full bg-white/90 sm:bg-zinc-100 text-zinc-600 hover:text-zinc-900 hover:bg-white transition-all shadow-md z-20"
        ...
        <div className="overflow-y-auto overscroll-contain flex-1">
          ...
          <div className="relative aspect-4/3 sm:aspect-16/10 w-full bg-zinc-100 overflow-hidden">
            ...
            <div className="absolute top-4 left-4 flex flex-col gap-1.5 z-10">
              {item.isPopular && (
                <div className="bg-red-500 text-white text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full shadow-md flex items-center gap-1">
                  <Sparkles className="w-3 h-3 fill-white" />
                  Bestseller
                </div>
              )}
              {isSoldOut && (
                <div className="bg-zinc-900/95 text-white text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full shadow-md border border-white/20 flex items-center gap-1.5">
          ...
          <div className="p-5 sm:p-6 space-y-4">
            ...
            {item.description ? (
              ...
              <p className="text-xs text-zinc-400 italic pt-1">Freshly prepared to order.</p>
            )}
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 bg-zinc-50/90 backdrop-blur-md border-t border-zinc-100 flex items-center gap-3 shrink-0">
          {isSoldOut ? (
            <button
              type="button"
              disabled
              className="w-full min-h-[48px] py-3.5 px-4 rounded-2xl font-bold text-sm bg-zinc-200 text-zinc-500 cursor-not-allowed flex items-center justify-center gap-2"
            >
